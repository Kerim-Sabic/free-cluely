...
/**
 * Create headers with optional JWT auth token
 */
...
export const CalendarPage: React.FC = () => {
  ...
  const loadConnections = useCallback(async () => {
    try {
      logger.debug('Loading calendar connections')

      const response = await fetch(`${API_BASE_URL}/api/calendar/connections`, {
        headers: createHeaders(token),
      })

      const data = await handleApiResponse<{ connections: CalendarConnection[] }>(response)

      if (data) {
        setConnections(data.connections || [])
        logger.info('Calendar connections loaded', { count: data.connections?.length || 0 })
      }
    } catch (err) {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      logger.debug('Loading calendar events')

      const response = await fetch(`${API_BASE_URL}/api/calendar/events?maxResults=20`, {
        headers: createHeaders(token),
      })

      const data = await handleApiResponse<{ events: CalendarEvent[] }>(response)

      if (data) {
        setEvents(data.events || [])
        logger.info('Calendar events loaded', { count: data.events?.length || 0 })
      }
    } catch (err) {
  ...
  const handleConnectGoogle = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/calendar/google/connect`, {
        headers: createHeaders(token),
      })

      const data = await handleApiResponse<{ authUrl: string }>(response)
      ...
      if (data?.authUrl) {
        ...
        const handleMessage = (event: MessageEvent) => {
          if (event.data.type === "calendar_connected") {
            logger.success('Google Calendar connected')
            loadConnections()
            loadEvents()
      ...
      logger.error('Failed to connect Google Calendar', err)
      setError({
        message: err instanceof Error ? err.message : 'Failed to connect calendar',
        timestamp: Date.now(),
      })
    } finally {
      setIsConnecting(false)
    }
  }, [token, isConnecting, loadConnections, loadEvents])
  ...
  const handleDisconnect = useCallback(async (provider: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/calendar/connection/${provider}`,
        {
          method: "DELETE",
          headers: createHeaders(token),
        }
      ...
      logger.success('Calendar disconnected', { provider })
      loadConnections()
      setEvents([])
      setSelectedEvent(null)
    } catch (err) {
      logger.error('Failed to disconnect calendar', err, { provider })
      setError({
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl mx-auto">
                ...
                <motion.button
                  ...
                  className="p-6 bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl hover:border-white/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isConnecting ? (
                  ...
                  <p className="text-sm text-gray-400">
                    {isConnecting ? 'Connecting...' : 'Connect your Google Calendar'}
                  </p>
                </motion.button>
